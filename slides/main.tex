...
\begin{frame}{Стабилизация ландшафта функции потерь и достаточный размер выборки}
    ...
    \begin{block}{Проблема}
        Эмпирические законы масштабирования для современных моделей наблюдаются на практике, но не имеют общего теоретического обоснования. Существующие критерии стабилизации ландшафта (одноточечный и изотропный среднеквадратичный) имеют верхние оценки с доминирующей константой, растущей как $N^2$ по размерности пространства параметров.
    \end{block}
    \begin{block}{Требуется}
        Построить критерий стабилизации поверхности функции потерь, согласованный с эмпирически наблюдаемой \alert{\textbf{анизотропией}} спектра матрицы Гессе, и получить количественные оценки достаточного размера обучающей выборки, применимые к нейронным сетям современных размеров.
    \end{block}
...
\begin{columns}[T]
    \begin{column}{0.57\textwidth}
        ...
        \begin{minipage}{\linewidth}
            ...
            \end{equation*}
            где $\mathbf{w} \in \mathcal{U}_{R}(\mathbf{w}_k^*)$ "--- точка из окрестности радиуса $R>0$ минимума $\mathbf{w}_k^{*}$, то есть $p=1$ и $q(\tilde{\mathbf{w}}) = \delta(\tilde{\mathbf{w}} - \mathbf{w})$.
        \end{minipage}
    \end{column}
    \hspace{0.2em}
    \begin{column}{0.4\textwidth}
...
\begin{columns}[T]
    \begin{column}{0.57\textwidth}
        ...
        \begin{minipage}{\linewidth}
            ...
            то есть $p=2$, для теоретического анализа удобно выбрать $q(\mathbf{w}) = \mathcal{N}(\mathbf{w}_k^*, \sigma^2 \mathbf{I}_N)$, а на практике вычисляется методом Монте"--~Карло через $\mathbf{w}_s \sim q(\mathbf{w})$:
            \vspace{-1.0em}
            \begin{equation*}
...
Общий шаг для всех алгоритмов "--- построение подпространства наибольшей кривизны $\mathbf U_D$ дефлированной степенной итерацией на произведениях матрицы Гессе на вектор (HVP).
\vspace{0.4em}

Лемма: в координатах $\mathbf z = \mathbf U_D^\top (\mathbf w - \mathbf w_k^*)$ критерий определяется тремя сжатыми объектами:
\vspace{-0.2em}
\[
...
\begin{table}[ht]
    \centering
    \footnotesize
    \begin{tabular}{@{}p{3.0cm} p{3.0cm} p{2.6cm} p{5.0cm}@{}}
        \toprule
        Алгоритм & Объект оценки & Настройка & Стоимость оценки \\
        \midrule
...
\end{table}
\vspace{-0.6em}
\begin{block}{Замкнутая форма через моменты гауссовской квадратичной формы}
    \vspace{-1.2em}
    \[
        \widehat \Delta_{2,\mathrm{GM}}^{(D)}(k+1) = a_k^2 + a_k \sigma^2 \operatorname{Tr}(\mathbf B_k) + \sigma^2 \|\mathbf c_k\|_2^2 + \tfrac{\sigma^4}{4}\bigl(2\operatorname{Tr}(\mathbf B_k^2) + \operatorname{Tr}(\mathbf B_k)^2\bigr).
    \]
\end{block}
\end{frame}
...
\begin{columns}[T]
    \begin{column}{0.6\textwidth}
        ...
        \begin{enumerate}
            ...
            \item Критерии вычисляются методом Монте"--~Карло (100 точек);
            \item Собственные векторы Гессе "--- степенной итерацией.
        \end{enumerate}
        Показатели степенного закона $\Delta(k) \sim k^{-\alpha}$ согласуются с теоретическим значением $\alpha = 2$ для $\Delta_2$ и $\Delta_2^{(D)}$.
    \end{column}
    \begin{column}{0.4\textwidth}
        \vspace{-1.0em}
        \begin{table}[htbp]
...
\begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=0.95\linewidth]{quadratic_sigma_loggrid_1e7_1e1_plot_abs.pdf}
        \end{figure}
    \end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Список работ автора по теме ВКР}
    \scriptsize
    {\usebeamercolor[fg]{block title} Препринт и публикации}\\
    \begin{enumerate}
        \item \textit{\textbf{Kiselev N.}, Grabovoy A.} Curvature-Aligned Probing for Local Loss-Landscape Stabilization // \textit{Preprint}, 2025. Submitted to NeurIPS 2026.
    ...
    \begin{enumerate}
        \item Устойчивая сходимость поверхности функции потерь через усреднение по распределению // \textit{Открытая конференция ИСП РАН}, 2025.
        \item Достаточный размер выборки и его связь со сходимостью поверхности функции потерь // \textit{22-я Всероссийская конференция <<Математические методы распознавания образов>>}, 2025.
